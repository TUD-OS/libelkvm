...
int elfloader_initialize_stack(struct elkvm_opts *opts, struct kvm_vm *vm) {
	...
	int bytes = elfloader_copy_and_push_str_arr_p(vm, target, opts->environ);
	target -= bytes;


	//followed by argv pointers
	printf("now push argv\n");
	bytes = elfloader_copy_and_push_str_arr_p(vm, target, opts->argv);
...
int elfloader_copy_and_push_str_arr_p(struct kvm_vm *vm, void *host_base_p,
	 	char **str) {
